...
def setup_repos():
    ...
    if not os.path.isdir('pulsar-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/PulsarCoin/Pulsar-Coin-Cryptocurrency-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('pulsar'):
        subprocess.check_call(['git', 'clone', 'https://github.com/PulsarCoin/Pulsar-Coin-Cryptocurrency.git'])
...
def build():
    ...
    os.makedirs('pulsar-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../pulsar/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])

    if args.linux:
        print('\nCompiling ' + args.version + ' Linux')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'pulsar='+args.commit, '--url', 'pulsar='+args.url, '../pulsar/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../gitian.sigs/', '../pulsar/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call('mv build/out/pulsar-*.tar.gz build/out/src/pulsar-*.tar.gz ../pulsar-binaries/'+args.version, shell=True)

    if args.windows:
        print('\nCompiling ' + args.version + ' Windows')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'pulsar='+args.commit, '--url', 'pulsar='+args.url, '../pulsar/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-unsigned', '--destination', '../gitian.sigs/', '../pulsar/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call('mv build/out/pulsar-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/pulsar-*.zip build/out/pulsar-*.exe build/out/src/pulsar-*.tar.gz ../pulsar-binaries/'+args.version, shell=True)
    ...
    if args.macos:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'pulsar='+args.commit, '--url', 'pulsar='+args.url, '../pulsar/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-unsigned', '--destination', '../gitian.sigs/', '../pulsar/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call('mv build/out/pulsar-*-osx-unsigned.tar.gz inputs/pulsar-osx-unsigned.tar.gz', shell=True)
        subprocess.check_call('mv build/out/pulsar-*.tar.gz build/out/pulsar-*.dmg ../pulsar-binaries/'+args.version, shell=True)
...
def sign():
    ...
    subprocess.check_call('cp inputs/pulsar-' + args.version + '-osx-unsigned.tar.gz inputs/pulsar-osx-unsigned.tar.gz', shell=True)
    subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../pulsar/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../pulsar/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    subprocess.check_call('mv build/out/pulsar-osx-signed.dmg ../pulsar-binaries/'+args.version+'/pulsar-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/pulsar-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/pulsar-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../pulsar/contrib/gitian-descriptors/gitian-linux.yml']):
        print('Verifying v'+args.version+' Linux FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' Windows\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../pulsar/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../pulsar/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    # TODO: Skip checking signed windows sigs until we actually start producing signed windows binaries
    #print('\nVerifying v'+args.version+' Signed Windows\n')
    #if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../pulsar/contrib/gitian-descriptors/gitian-win-signer.yml']):
    #    print('Verifying v'+args.version+' Signed Windows FAILED\n')
    #    rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../pulsar/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/PulsarCoin/Pulsar-Coin-Cryptocurrency', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/pulsar'):
            os.makedirs('../gitian-builder/inputs/pulsar')
        os.chdir('../gitian-builder/inputs/pulsar')
