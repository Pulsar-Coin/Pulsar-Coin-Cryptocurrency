 ...
 * between pulsard, and pulsar-qt and non-server tools.
*/
...
/** Sanity checks
 *  Ensure that Pulsar is running in a usable environment with all
...
{
    // Make sure only a single Pulsar process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // pulsar: init hash seed
    pulsarRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // pulsar: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // pulsar: checkpoint master priv key
    {
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if pulsar is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if pulsar is started while in a temporary directory.\n",
...
#ifdef ENABLE_CHECKPOINTS
                // pulsar: initialize synchronized checkpoint
                if (!fReindex && !WriteSyncCheckpoint(chainparams.GenesisBlock().GetHash()))
                    return error("LoadBlockIndex() : failed to init sync checkpoint");

                // pulsar: if checkpoint master key changed must reset sync-checkpoint
    ...
    GeneratePulsar(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-gen", DEFAULT_GENERATE_THREADS), chainparams);
#endif
