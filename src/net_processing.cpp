...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (IsHalvingActive(chainActive.Tip(), Params().GetConsensus()) && nVersion < HALVING_VERSION) {
            ...
            connman->PushMessage(pfrom, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::REJECT, strCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater", HALVING_VERSION)));
            pfrom->fDisconnect = true;
            return false;
        }
        if (IsHalvingActiveFix(chainActive.Tip(), Params().GetConsensus()) && nVersion < HALVING_FIX_VERSION) {
            // disconnect from peers older than this proto version
