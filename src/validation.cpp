...
const std::string strMessageMagic = "Pulsar Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // pulsar: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
int64_t GetProofOfWorkReward(unsigned int nHeight) {
    ...
    else if (nHeight > 1 && nHeight <= 2500000) {
    return 90 * COIN;
    }
    ...
    else if (nHeight > 10000000 && nHeight <= 12500000) {
    return 5.625 * COIN;
    }
    else if (nHeight > 12500000 && nHeight <= 15000000) {
    return 2.8125 * COIN;
    }
    else if (nHeight > 15000000 && nHeight <= 17500000) {
    return 1.40625000 * COIN;
    }
    else if (nHeight > 17500000 && nHeight <= 20000000) {
    return 0.70312500 * COIN;
    }
    else if (nHeight > 20000000 && nHeight <= 22500000) {
    return 0.35156290 * COIN;
    }
    else if (nHeight > 22500000 && nHeight <= 25000000) {
    return 0.17578125 * COIN;
    }
    else if (nHeight > 25000000 && nHeight <= 27500000) {
    return 0.08789063 * COIN;
    }
    else if (nHeight > 27500000 && nHeight <= 30000000) {
    return 0.04394531 * COIN;
    }
    else if (nHeight > 30000000 && nHeight <= 32500000) {
    return 0.02197266 * COIN;
    }
    else if (nHeight > 32500000 && nHeight <= 35000000) {
    return 0.01098633 * COIN;
    }
    else if (nHeight > 35000000 && nHeight <= 37500000) {
    return 0.00549316 * COIN;
    }
    else if (nHeight > 37500000 && nHeight <= 40000000) {
    return 0.00274658 * COIN;
    }
    else if (nHeight > 40000000 && nHeight <= 42500000) {
    return 0.00137329 * COIN;
    }
    else if (nHeight > 42500000 && nHeight <= 45000000) {
    return 0.00068665 * COIN;
    }
    else if (nHeight > 45000000 && nHeight <= 47500000) {
    return 0.00034332 * COIN;
    }
    else if (nHeight > 47500000 && nHeight <= 50000000) {
    return 0.00017166 * COIN;
    }
    else if (nHeight > 50000000 && nHeight <= 52500000) {
    return 0.00008583 * COIN;
    }
    else if (nHeight > 52500000 && nHeight <= 55000000) {
    return 0.00004292 * COIN;
    }
    else if (nHeight > 55000000 && nHeight <= 57500000) {
    return 0.00002146 * COIN;
    }
    else if (nHeight > 57500000 && nHeight <= 60000000) {
    return 0.00001073 * COIN;
    }
    else if (nHeight > 60000000 && nHeight <= 62500000) {
    return 0.00000536 * COIN;
    }
    else if (nHeight > 62500000 && nHeight <= 65000000) {
    return 0.00000268 * COIN;
    }
    else if (nHeight > 65000000 && nHeight <= 67500000) {
    return 0.00000134 * COIN;
    }
    else if (nHeight > 67500000 && nHeight <= 70000000) {
    return 0.00000067 * COIN;
    }
    else if (nHeight > 70000000 && nHeight <= 72500000) {
    return 0.00000034 * COIN;
    }
    else if (nHeight > 72500000 && nHeight <= 75000000) {
    return 0.00000017 * COIN;
    }
    else if (nHeight > 75000000 && nHeight <= 77500000) {
    return 0.00000008 * COIN;
    }
    else if (nHeight > 77500000 && nHeight <= 80000000) {
    return 0.00000004 * COIN;
    }
    else if (nHeight > 80000000 && nHeight <= 82500000) {
    return 0.00000002 * COIN;
    }
    else if (nHeight > 82500000 && nHeight <= 85000000) {
    return 0.00000001 * COIN;
    }
    else {
    return 0.00000001 * COIN;
    }
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // pulsar: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // pulsar
            undo.nTime = alternate.nTime;           // pulsar
        } else {
...
bool PulsarContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // pulsar: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!PulsarContextualBlockChecks(block, state, pindex, fJustCheck))
  ...
  //  if (block.IsProofOfWork() && (pindex->nHeight > 0 && pindex->pprev->nPOWBlockHeight + 1 > params.nTotalPOWBlock))
    ...
    // pulsar: coinbase reward check relocated to CheckBlock()
    ...
    // pulsar: track money supply and mint amount info
    ...
    // pulsar: fees are not collected by miners as in bitcoin
    // pulsar: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // pulsar: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // pulsar: first coinbase output should be empty if proof-of-stake block
...
#ifdef ENABLE_CHECKPOINTS
    // pulsar: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // pulsar: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // pulsar: check PoS
    if (fCheckPoS && !PulsarContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // pulsar: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // pulsar: if responsible for sync-checkpoint send it
...
{
	int nHighest = 1;
    if (!blocktree.LoadBlockIndexGuts(consensus_params, [this](const uint256& hash, nHighest){ return this->InsertBlockIndex(hash); }))
        return false;
...
#ifdef ENABLE_CHECKPOINTS
    // pulsar: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // pulsar: txindex is always enabled
...
// pulsar: total coin age spent in transaction, in the unit of coin-days.
